...
from linear_relational.lib.layer_matching import get_layer_name
...
@dataclass
class LogisticRegressionConceptTrainerOptions(ConceptTrainerOptions):
...
class LogisticRegressionConceptTrainer(
    ConceptTrainer[LogisticRegressionConceptTrainerOptions]
):
    @override
    def train_relation_concepts(
        self,
        relation: str,
        opts: LogisticRegressionConceptTrainerOptions,
        verbose: bool = True,
    ) -> list[Concept]:
    ...
    def _train_concept(
        ...
        vector = train_concept_vector_via_logistic_regression(
